...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$penduduk_id || !$tgl_pindah) {
        $error = 'Pilih penduduk dan tanggal pindah!';
    } else {
        $pdo->beginTransaction();
        try {
            $stmt = $pdo->prepare("INSERT INTO pindah (penduduk_id, tgl_pindah, alamat_tujuan, alasan_pindah, keterangan) VALUES (?,?,?,?,?)");
            $stmt->execute([$penduduk_id, $tgl_pindah, $alamat_tujuan, $alasan_pindah, $keterangan]);

            $stmt = $pdo->prepare("UPDATE penduduk SET status_hidup = 'pindah' WHERE id = ?");
            $stmt->execute([$penduduk_id]);

            $pdo->commit();
            $_SESSION['flash'] = ['type' => 'success', 'msg' => 'Data pindah berhasil dicatat!'];
            redirect('index.php');
        } catch (Exception $e) {
            $pdo->rollBack();
...
<head>
    ...
    <title>Tambah Pindah - <?= htmlspecialchars($profil['nama_desa']) ?></title>
    ...
    <div class="main-content">
        ...
        <div class="content-wrapper">
            <div class="page-header d-flex justify-content-between align-items-center">
                <div>
                    <h4>Tambah Pindah</h4>
                    <p>Catat penduduk yang pindah</p>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" class="row g-3">
                        ...
                        <div class="col-12">
                            <button type="submit" class="btn btn-warning text-dark"><i class="bi bi-save"></i> Simpan</button>
